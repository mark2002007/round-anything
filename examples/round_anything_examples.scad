...
// translate_radii_pointsExample();
// 2dShellExample();
// beam_chainExample();
// mirror_pointsExample();
// radiusExtrudeExample();
// polyround_extrudeExample();
...
module basicpolyroundExample(){
  ...
  %translate([0,0,0.3])polygon(get_points(radiiPoints));//transparent copy of the polgon without rounding
}
...
module parametricpolyroundExample() {
  ...
  %translate([-25,0,0.2])polygon(get_points(points));//transparent copy of the polgon without rounding
}
...
module experimentalParametricpolyroundExample() {
  ...
  %translate([-50,0,0.2])polygon(get_points(process_radii_points(points2)));//transparent copy of the polgon without rounding
}
...
module translate_radii_pointsExample() {
...
module beam_chainExample(){
  ...
  translate([0,0,0]){
    ...
    linear_extrude(1)polygon(polyround(beam_chain(radiiPoints,offset1=0.02, offset2=-0.02),20));
  }
  ...
  translate([0,-7*2,0]){
    radiiPoints=beamPoints(2,1);
    linear_extrude(1)polygon(polyround(beam_chain(radiiPoints,offset1=0.5, offset2=-0.5),20));
  }

  // Add an angle to the start of the beam
  translate([0,-7*3,0]){
    radiiPoints=beamPoints(2,1);
    linear_extrude(1)polygon(polyround(beam_chain(radiiPoints,offset1=0.5, offset2=-0.5, startAngle=45),20));
  }
  ...
  translate([0,-7*9,0]){
    radiiPoints=beamPoints(2,1,rEnd=3);
    for(i=[0:2]){linear_extrude(1)polygon(polyround(beam_chain(radiiPoints,offset1=-1+i*0.4, offset2=-1+i*0.4+0.25),20));}
  }
}

module mirror_pointsExample(){
  function points(endR=0)=[[0,0,0],[2,8,0],[5,4,3],[15,10,0.5],[10,2,endR]];
  mirroredPoints=mirror_points(points(0),0,[1,0]);
  polygon(polyround(mirroredPoints,20));
  mirroredPoints2=mirror_points(points(7),0,[1,0]);
  translate([0,-20,0])polygon(polyround(mirroredPoints2,20));
}

module radiusExtrudeExample(){
  radiiPoints=[[-4,0,1],[5,3,1.5],[0,7,0.1],[8,7,10],[20,20,0.8],[10,0,10]];
  extrude_with_radius(3,0.5,0.5,50)polygon(polyround(radiiPoints,30));
  #translate([7,4,3])extrude_with_radius(3,-0.5,0.95,50)circle(1,$fn=30);
}

module polyround_extrudeExample(){
  radiiPoints=[[10,0,10],[20,20,1.1],[8,7,10],[0,7,0.3],[5,3,0.1],[-4,0,1]];
  polyround_extrude(radiiPoints,2,0.5,-0.8,fn=8);
}
...
module testGeometries() {
  ...
  polyround_extrudeTestShape(points);
  translate([0,20,0])polyround_extrudeTestShape(reversedPoints);
  ...
  translate([20,0,0])linear_extrude(1)polygon(polyround( beam_chain(issue16pointsa, offset1=1, offset2=-1), 30));
  
  didBreakWhen0b=1e-6;
  issue16pointsb=[[0, 0, 0], [0+didBreakWhen0b, 10, 0], [10, 10+didBreakWhen0b, 0]];
  translate([20,15,0])linear_extrude(1)polygon(polyround( beam_chain(issue16pointsb, offset1=1, offset2=-1), 30));

}

module polyround_extrudeTestShape(points) {
  // make sure no faces are inverted
  difference() {
    translate([0, 0, -2.5]) polyround_extrude(points,r1=-1,r2=1);
    sphere(d=9);

...
minkowski_round(outsideRadius,internalRadius,enable,envelopecube[x,y,z]) -> takes 3d children
...
Both this modules do the same thing as minkowski_round() but focus on either inside or outside radiuses, which means these modules use one less minkowski() (two instead of three) making them quicker if you don't want both inside and outside radiuses.
...
// $fn=20;
// minkowski_round(0.7,1.5,1,[50,50,50])union(){//--example in the thiniverse thumbnail/main image
//   cube([6,6,22]);
...
module minkowski_round(OR=1,IR=1,enable=1,boundingEnvelope=[500,500,500]){
...
module minkowski_outside_round(r=1,enable=1,boundingEnvelope=[500,500,500]){
  if(enable==0){//do nothing if not enabled
    children();
  } else {
...
module minkowski_inside_round(r=1,enable=1,boundingEnvelope=[500,500,500]){
  if(enable==0){//do nothing if not enabled
    children();
  } else {

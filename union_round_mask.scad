...
// end of demo code
//
module union_round_mask(r=1, detail = 5,q=70, epsilon = 1e-6, showMask = false, includeOperands = true) {
    //automask if none
    if($children <=2){
...
function spherical_coordinate(x,y)=  [cos(x  )*cos(y  ), sin(x  )*cos(y  ), sin(y  )];
...
spherical_coordinate(   s*x*toDeg,  PI/2.* sign(s)*(1.-sqrt(1.-abs(s)))*toDeg)];
